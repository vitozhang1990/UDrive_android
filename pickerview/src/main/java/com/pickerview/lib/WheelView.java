...
public class WheelView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		//
		// if (label != null) {
		// int drawRightContentStart = measuredWidth / 2 +
		...
		while (counter < itemsVisible) {
			...
			if (angle >= 90F || angle <= -90F) {
				...
				String contentTextStr = getContentText(visibles[counter]);
				String contentText = "";
				if (getContentText(visibles[counter]) == null) {
					Log.e("getContentText", "==NULL===" + contentText);
				}
				if (label != null &&label.length()>0 && getContentText(visibles[counter]) != null&& getContentText(visibles[counter]).length()>0) {
					contentText = contentTextStr + label + "";
					Log.e("getContentText", "===YES==" + contentText);
				} else {
					contentText = contentTextStr;
					Log.e("getContentText", "===NO==" + contentText);
				}
				// 计算开始绘制的位置
				...
				if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
					...
					// Log.e("preCurrentIndex", "====" + preCurrentIndex);
				} else {
					// 其他条目
					canvas.save();
					canvas.clipRect(0, 0, measuredWidth, (int) (itemHeight));
	...
	private int getLoopMappingIndex(int index) {
		...
		Log.e("getLoopMappingIndex", "====" + index);
		return index;
	}
