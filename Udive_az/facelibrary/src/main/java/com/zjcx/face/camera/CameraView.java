...
import com.zjcx.face.R;
import com.zjcx.face.util.DimensionUtil;
import com.zjcx.face.util.ImageUtil;
...
public class CameraView extends FrameLayout {
    ...
    public static final int NATIVE_DRIVING_LICENSE_FRONT = 13;
    public static final int NATIVE_DRIVING_LICENSE_BACK = 14;
    public static final int NATIVE_ID_CARD_FRONT = 15;
    public static final int NATIVE_ID_CARD_BACK = 16;
    /**
     * 是否已经通过本地质量控制扫描
     */
    ...
    public void setMaskType(@MaskView.MaskType int maskType, final Context ctx) {
        ...
        switch (maskType) {
            ...
            case MaskView.MASK_TYPE_ID_CARD_BACK:
                ...
                hintResourceId = R.drawable.bd_ocr_round_corner;
                break;
            case MaskView.MASK_TYPE_DRIVING_LICENSE_FRONT:
                isNeedSetImage = false;
                hintResourceId = R.drawable.bd_ocr_round_corner;
                break;
            case MaskView.MASK_TYPE_DRIVING_LICENSE_BACK:
                isNeedSetImage = false;
                hintResourceId = R.drawable.bd_ocr_round_corner;
                break;
        ...
        if ((maskType == MaskView.MASK_TYPE_DRIVING_LICENSE_FRONT)||(maskType == MaskView.MASK_TYPE_DRIVING_LICENSE_BACK) && isEnableScan) {
            cameraControl.setDetectCallback(new ICameraControl.OnDetectPictureCallback() {
                @Override
                public int onDetect(byte[] data, int rotation) {
                    return detect(data, rotation);
...
//                status = IDcardQualityProcess.getInstance().idcardQualityDetectionImg(bitmap, true);
                status = NATIVE_ID_CARD_FRONT;
                break;
            case MaskView.MASK_TYPE_ID_CARD_BACK:
//                status = IDcardQualityProcess.getInstance().idcardQualityDetectionImg(bitmap, false);
                status = NATIVE_ID_CARD_BACK;
                break;
            case MaskView.MASK_TYPE_DRIVING_LICENSE_FRONT:
                status = NATIVE_DRIVING_LICENSE_FRONT;
                break;
            case MaskView.MASK_TYPE_DRIVING_LICENSE_BACK:
                  status = NATIVE_DRIVING_LICENSE_BACK;
                break;
            default:
                status = 1;
        }
    ...
    private String getScanMessage(int status) {
        ...
        switch (status) {
            ...
            case NATIVE_SOLOAD_FAIL:
                message = "本地SO库加载失败";
                break;
            case NATIVE_DRIVING_LICENSE_FRONT:
                message="请将驾驶证人像面置于此区域，保证信息清晰，拍照";
                break;
            case NATIVE_DRIVING_LICENSE_BACK:
                message="请将驾驶证副页置于此区域，保证信息清晰，拍照";
                break;
            case NATIVE_ID_CARD_FRONT:
                message="请将身份证人像面置于此区域，保证信息清晰，拍照";
                break;
            case NATIVE_ID_CARD_BACK:
                message="请将身份证国徽面置于此区域，保证信息清晰，拍照";
                break;
            case 1:
            default:
                message = "请将身份证置于取景框内";
...
//        hintViewText.setBackgroundResource(R.drawable.bd_ocr_round_corner);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int hintViewWidth = DimensionUtil.dpToPx(right);
